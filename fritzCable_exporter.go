...
func main() {
	...
	exporter, err := NewExporter(*clientScrapeURI, *clientUsername, *clientPassword, *clientTimeout)
	if err != nil {
		log.Fatal(err)
	}
	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		w.Write([]byte(`<html>
             <head><title>Fritzbox Cable Exporter</title></head>
             <body>
             <h1>Fritzbox Cable Exporter</h1>
